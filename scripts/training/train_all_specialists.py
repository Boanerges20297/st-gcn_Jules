...
class SpecialistTrainer:
    def __init__(self, region_key, epochs=50, lr=0.05, grad_accum=16, dropout=0.5, ranking_weight=20.0):
    ...
    def train(self):
        ...
        logging.info("\n" + "="*80)
        logging.info(f"🚀 INICIANDO TREINAMENTO ELITE: {region_label}")
        logging.info("="*80)
        ...
        def criterion(pred, target, info):
            ...
            k = 20 if self.region_key == 'fortaleza' else 10
            top_val, top_idx = torch.topk(target, min(k, len(target)))
            w = spatial_weights.clone()
            w[top_idx] = w[top_idx] * 5.0 * (1.0 + target[top_idx])
            ...
            margin = 0.25 + (F.relu(t_h - t_l) * 0.5)
        ...
        logging.info(f"🎬 Iniciando Treinamento: {self.epochs} épocas")
        total_batches = len(train_X) // self.grad_accum
        ...
        for epoch in range(self.epochs):
            ...
            for idx in indices:
                ...
                if steps % self.grad_accum == 0:
                    ...
                    if current_batch % 10 == 0 or current_batch == 1:
                        with torch.no_grad():
            ...
            # Validation on Reality Set (Lastro)
            model.eval()
            p10_l, p20_l = [], []
            with torch.no_grad():
...
def main():
    ...
    tasks = [
        ('fortaleza', 50, 0.05, 16, 0.5, 20.0),
        ('rmf', 60, 0.03, 8, 0.4, 25.0),
        ('interior', 60, 0.03, 8, 0.4, 25.0)
    ]
