...
try:
    from src.core.architectures import DeepSTGAT_32 
except ImportError:
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("logs/training_interior_elite.log", mode='w', encoding='utf-8'),
...
def train_specialist(region_key, ModelClass):
    ...
    logging.info(f"⚡ ESTRATÉGIA ELITE INTERIOR: {region_label}")
    ...
    # JANELA AMPLA PARA ELIMINAR RUÍDO (Padrão Elite)
    ...
    # DROPOUT 0.3 para Interior (Muitos nós, precisa de alguma regularização)
    ...
    def criterion(pred, target, info):
        ...
        # FOCO EM P@10 (Média de ~160 municípios)
        k = 10 
        ...
        num_neg = 25 # Mais negativos por ser um dataset maior
        ...
        # RANKING 25.0 para consolidar o Interior
    ...
    logging.info(f"🎬 Iniciando Treino Interior ELITE: {EPOCHS} épocas | Foco P@10/P@20")
    logging.info(f"{'='*80}")
    ...
    for epoch in range(EPOCHS):
        ...
        real_p20 = np.mean(p20_l or [0])
        combined_score = (real_p10 * 0.6) + (real_p20 * 0.4)
        
        logging.info(f"🏁 Epoch {epoch+1:03d} | REALITY P@10: {real_p10*100:.1f}% | P@20: {real_p20*100:.1f}%")
        
        if combined_score > best_combined:
            best_combined = combined_score
            save_path = os.path.join(ROOT_DIR, 'models', 'active', f'interior_model_elite.pth')
            torch.save({
            ...
            logging.info(f"🏆 NOVO RECORDE INTERIOR (P@10): {real_p10*100:.1f}% | (P@20): {real_p20*100:.1f}%")
        
        if real_p10 >= 0.70 and real_p20 >= 0.80:
            logging.info(f"🎯 META ALCANÇADA: P@10={real_p10*100:.1f}%, P@20={real_p20*100:.1f}%")
            break

if __name__ == "__main__":
    from src.core.architectures import DeepSTGAT_64
    train_specialist('interior', DeepSTGAT_64)
