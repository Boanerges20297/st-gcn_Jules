...
# Configuração de Logging focado em ELITE P10 (Estabilizado)
os.makedirs('logs', exist_ok=True)
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s [%(levelname)s] %(message)s',
    handlers=[
        logging.FileHandler("logs/training_ELITE_P10.log", mode='w', encoding='utf-8'),
        logging.StreamHandler(sys.stdout)
    ]
)

# --- HIPERPARÂMETROS ESTABILIZADOS ---
EPOCHS = 45 
LR_MIN = 0.0001
LR_MAX = 0.018 
WINDOW = 90
...
# Reduzido de 16 para 8 para diminuir pressão na memória por batch
...
def train_region(region_key):
    logging.info(f"\n" + "="*60)
    logging.info(f"🚀 INICIANDO TREINO ELITE P10 (ESTABILIZADO): {region_key.upper()}")
    logging.info("="*60)
    ...
    for epoch in range(EPOCHS):
        ...
        # Limpeza agressiva de memória a cada época
        if DEVICE.type == 'cuda':
        ...
        with open(f'logs/metrics_{region_key}_ELITE.json', 'w') as f:
...
def main():
    # Limpar processos python anteriores para garantir memória limpa
