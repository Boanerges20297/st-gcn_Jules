...
# Configurações de Treino (ESTRESSE T12)
EPOCHS = 25
LR = 0.02 
...
def train_specialist(region_key, ModelClass):
    ...
    logging.info(f"="*50)
    logging.info(f"⚡ INICIANDO TREINAMENTO: {region_label} (AGRESSIVE DYNAMIC)")
    logging.info(f"="*50)
    ...
    for epoch in range(EPOCHS):
        ...
        if real_p20 > best_p20:
            ...
            logging.info(f"🏆 NOVO RECORDE REALIDADE: {real_p20*100:.1f}%")
