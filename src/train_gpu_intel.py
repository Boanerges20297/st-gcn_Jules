"""
Script para treino com GPU Intel Iris via oneAPI.
Usa Mixed Precision (float16) e XPU (Intel GPU) se disponível.
"""
import os
import sys
import time
...
def get_device():
    """Detectar melhor dispositivo disponível."""
    logger = setup_logger()
    
    # Tentar XPU (Intel GPU via oneAPI)
    try:
        import intel_extension_for_pytorch as ipex
        if torch.xpu.is_available():
            logger.info('Intel XPU (GPU Iris) detectada via IPEX!')
            return torch.device('xpu'), True, 'xpu'
    except ImportError:
        pass
    except Exception as e:
        logger.warning(f'XPU não disponível: {e}')
    
    # Fallback: CPU com otimizações
    logger.info('Usando CPU otimizado com Mixed Precision')
...
def train_one(model, loader, optimizer, device, norm_adj, loss_weights, logger, epoch, device_type='cpu', scaler=None, print_every=10):
    model.train()
    total_loss = 0.0
    batch_times = []
    ...
    for batch_idx, (bx, by) in enumerate(loader, 1):
        ...
        # Mixed Precision com device correto
        if device_type == 'xpu':
            ...
            # CPU: sem autocast, direto com float32 (mais rápido que autocast no CPU)
            out = model(bx, norm_adj)
            loss = weighted_mse_loss(out, by, loss_weights)
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=5.0)
...
def run_training(history_window=30, horizon=1, epochs=50, batch_size=128, lr=1e-3, out_path=None):
    logger = setup_logger()
    ...
    if dates is not None:
        ...
        logger.info(f'Subsetting from 2024-01-01 (index {start_idx}). New timesteps: {node_features.shape[1]}')
    ...
    device, use_gpu, device_type = get_device()
    logger.info(f'Using device: {device} (GPU={use_gpu})')
    ...
    # GradScaler apenas se usar GPU XPU
    scaler = GradScaler() if device_type == 'xpu' else None
    ...
    for epoch in range(1, epochs+1):
        t0 = time.time()
        train_loss = train_one(model, train_loader, optimizer, device, norm_adj, loss_weights, logger, epoch, device_type, scaler)
        ...
        if val_loss < best_val:
            ...
            logger.info(f'Checkpoint saved: {out_path} (val_loss improved)')
        else:
            no_improve_count += 1
...
if __name__ == '__main__':
    logger = setup_logger()
    logger.info('=== Treino com suporte a GPU Intel Iris ===')
    
    out1 = os.path.join(ROOT, 'models', 'stgcn_30_1.pth')
    out7 = os.path.join(ROOT, 'models', 'stgcn_30_7.pth')
    
    try:
        run_training(history_window=30, horizon=1, epochs=50, batch_size=128, lr=1e-3, out_path=out1)
        run_training(history_window=30, horizon=7, epochs=50, batch_size=128, lr=1e-3, out_path=out7)
        logger.info('✓ Treino completado com sucesso!')
    except Exception as e:
