...
def prepare_dataset(node_features, history_window, horizon):
    ...
    for i in range(0, num_timesteps - history_window - horizon + 1):
        ...
        # target is sum over next `horizon` days
...
def run_training(history_window=30, horizon=1, epochs=3, batch_size=64, lr=1e-3, out_path=None):
    logger = setup_logger()
    ...
    # Subset dates from 2024-01-01 onwards (as requested)
    if dates is not None:
        ...
        logger.info(f'Subsetting dates from index {start_idx}. New timesteps: {node_features.shape[1]}')

    N, T, F = node_features.shape
    logger.info(f'Data shapes -> nodes: {N}, timesteps: {T}, features: {F}')

    X, Y = prepare_dataset(node_features, history_window, horizon)
    if len(X) == 0:
        raise RuntimeError('Not enough timesteps for given history_window and horizon')
    ...
    # Use num_workers para paralelizar carregamento de dados
    train_loader = DataLoader(TensorDataset(torch.from_numpy(X_train), torch.from_numpy(Y_train)), batch_size=batch_size, shuffle=True, num_workers=0, pin_memory=False)
    val_loader = DataLoader(TensorDataset(torch.from_numpy(X_val), torch.from_numpy(Y_val)), batch_size=batch_size, shuffle=False, num_workers=0, pin_memory=False)

    device = torch.device('cpu')
    # Usar multi-threading da CPU para melhor performance
    torch.set_num_threads(8)
    logger.info(f'Using device: {device} with {torch.get_num_threads()} threads')
    ...
    for epoch in range(1, epochs+1):
        ...
        if val_loss < best_val:
            ...
            logger.info(f'Checkpoint saved to {out_path} (val_loss improved)')
        else:
            no_improve_count += 1
...
if __name__ == '__main__':
    # Checkpoints 30->1 e 30->7, batch_size=128, patience=15 (early stopping)
